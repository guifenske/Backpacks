...
public class BpgiveID implements CommandExecutor, TabCompleter {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(args.length != 2){
            sender.sendMessage(Main.PREFIX + "§cUsage: /bpgiveid <player> <backpackId>");
            return true;
        }
        ...
        if(!Main.backPackManager.getBackpacks().containsKey(Integer.parseInt(args[1]))){
            sender.sendMessage(Main.PREFIX + "§cBackpack not found!");
            return true;
        }

        BackPack backPack = Main.backPackManager.getBackpacks().get(Integer.parseInt(args[1]));
