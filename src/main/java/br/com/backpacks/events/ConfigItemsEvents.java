...
public class ConfigItemsEvents implements Listener {
    ...
    private void onPlace(BlockPlaceEvent event){
        if(event.getItemInHand().getItemMeta().getPersistentDataContainer().has(UpgradesRecipes.isUpgrade(), PersistentDataType.INTEGER)){
    ...
    private void onEat(PlayerItemConsumeEvent event){
        if(event.getItem().getItemMeta().getPersistentDataContainer().has(UpgradesRecipes.isUpgrade(), PersistentDataType.INTEGER)){
            event.setCancelled(true);
        }
    }
