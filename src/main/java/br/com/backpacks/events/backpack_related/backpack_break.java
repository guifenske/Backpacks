...
public class backpack_break implements Listener {
    ...
    private void general_break_backpack_chest_event(BlockBreakEvent event){
        ...
        if(Main.backPackManager.get_backpack_from_location(event.getBlock().getLocation()) == null) return;

        event.setDropItems(false);

        BackPack backPack = Main.backPackManager.get_backpack_from_location(event.getBlock().getLocation());
